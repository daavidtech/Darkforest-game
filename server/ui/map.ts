...
const MapView = () => {
    ...
    const isInProgress = (b: PlacedBuilding) => {
        ...
        const item = q.find((i) => i.origin.x === b.origin.x && i.origin.y === b.origin.y && i.building.name === b.building.name)
    ...
    const applyOutline = (b: PlacedBuilding) => {
        ...
        const base = getComputedStyle(document.documentElement).getPropertyValue('--border') || 'var(--border)'
        b.building.shape.forEach(({ x, y }) => {
            const cx = b.origin.x + x
            const cy = b.origin.y + y
            if (!inBounds(cx, cy)) return
    ...
    const place = (building: Building, ox: number, oy: number) => {
        ...
        const pb: PlacedBuilding = { building, origin: { x: ox, y: oy }, owner: "self" }
        placed.push(pb)
        renderBuilding(pb)
    ...
    const placeEnemy = (building: Building, ox: number, oy: number) => {
        ...
        building.shape.forEach(({ x, y }) => occupied.add(makeKey(ox + x, oy + y)))
        const pb: PlacedBuilding = { building, origin: { x: ox, y: oy }, owner: "enemy" }
        placed.push(pb)
        renderBuilding(pb)
        return true
    }
    ...
    window.addEventListener("mouseup", () => { isPanning = false; viewport.style.cursor = "" })
    ...
    controls.appendChild(btn("-", () => { scale = clamp(scale * 0.9, 0.4, 3); applyTransform() }))
    controls.appendChild(btn("+", () => { scale = clamp(scale * 1.1, 0.4, 3); applyTransform() }))
    controls.appendChild(btn("Reset", () => { scale = 1; tx = 0; ty = 0; applyTransform() }))
    ...
    ;(window as any).DF_addEnemy = (name: string, x: number, y: number) => {
        const b = allBuildings.find((bb) => bb.name.toLowerCase() === String(name).toLowerCase())
        if (!b) return false
